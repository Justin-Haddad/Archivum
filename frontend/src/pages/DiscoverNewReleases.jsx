    ...
    const fetchNewReleases = async () => {
      if (mediaType === 'all' || mediaType === 'movies') {
        ...
        try {
          ...
          toast.error('Failed to load new movies')
        } finally {
      ...
      if (mediaType === 'all' || mediaType === 'tv_shows') {
        ...
        try {
          ...
          toast.error('Failed to load new TV shows')
        } finally {
            ...
            <div className="modal" onClick={(e) => e.stopPropagation()}>
              ...
              <div style={{ textAlign: 'center', marginBottom: 'var(--space-xl)' }}>
                ...
                <div style={{ marginBottom: 'var(--space-xl)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 'var(--space-sm)' }}>
                  <input
                    type="checkbox"
                    id="want-to-watch-new-releases"
                    checked={selectedWantToWatch}
                    onChange={(e) => setSelectedWantToWatch(e.target.checked)}
                    style={{ width: '18px', height: '18px', cursor: 'pointer' }}
                  />
                  <label htmlFor="want-to-watch-new-releases" style={{ cursor: 'pointer', fontSize: '1rem', color: 'var(--color-gray-dark)' }}>
                    Add to Backlog
                  </label>
                </div>
              </div>
